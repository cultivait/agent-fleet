// windows.mjs — D5 fleet launcher, Windows lane (D5-W, task_8b3d40af3d).
// OWNER: linux-b37c. fleet.mjs imports { WIN_SSH_ALIAS, preflightWindows, spawnWindows }.
//
// Contract: d5-build-plan.md §4. Grounded Windows facts: d5-build-plan.md §1.
//   - ssh `user` lands in cmd.exe (user winuser); claude = C:\Users\winuser\.local\bin\claude.exe v2.1.179
//   - node v22.18.0 (single, clean); NO tmux / NO WSL distro → detach via PowerShell Start-Process, NOT tmux
//   - join token already in settings.json (Approach C) → do NOT inject it over ssh
//   - SessionStart hook radio-hook.js sets the sid-derived callsign `windows-<sid[0:4]>` + INJECTS a
...
//     never joins (proven on the live box: linux-09a8 spawned, title set, never on the roster). FIX:
...
export const WIN_SSH_ALIAS = process.env.WT_WIN_SSH_ALIAS || 'user';

// claude.exe location on the Windows box (grounded §1); overridable for testing/other hosts.
const WIN_CLAUDE_BIN = process.env.WT_WIN_CLAUDE_BIN || 'C:\\Users\\winuser\\.local\\bin\\claude.exe';
 ...
 * Windows-style path (drive letter). Otherwise fall back to the winuser profile.
 * Override with WT_WIN_WORK_DIR. (Per-node work dirs are a clean v2; this guard is right for v1.)
 */
export function winWorkDir(ctx = {}) {
  // First valid Windows-style path wins (env override, then ctx); else the winuser profile.
...
/**
 * Lane B (D5-W): spawn `count` detached Claude Code sessions on Windows.
 * Each is launched hidden+detached via `ssh user → powershell Start-Process`; the token comes
