...
// Pure-helper + dry-run coverage for the Windows lane.
// No live spawns: only --dry-run + pure fns.
...
test('WIN_SSH_ALIAS: defaults to windows-node', () => {
  assert.equal(WIN_SSH_ALIAS, 'windows-node');
});
...
test('winWorkDir: Linux ctx.workDir falls through to the Windows default', () => {
  ...
  assert.equal(winWorkDir({ workDir: '/home/user' }), 'C:\\Users\\<username>');
  assert.equal(winWorkDir({}), 'C:\\Users\\<username>');
});

test('winWorkDir: honors an explicit Windows-style ctx.workDir', () => {
  assert.equal(winWorkDir({ workDir: 'D:\\work\\fleet' }), 'D:\\work\\fleet');
  assert.equal(winWorkDir({ workDir: 'C:/Users/<username>/proj' }), 'C:/Users/<username>/proj');
});
...
test('windowsPsScript: hidden detached Start-Process with PassThru + PID echo', () => {
  const s = windowsPsScript('C:\\Users\\<username>');
  ...
  assert.match(s, /-WorkingDirectory 'C:\\Users\\<username>'/);
...
test('encodePsCommand: round-trips as UTF-16LE base64 (what -EncodedCommand expects)', () => {
  const script = windowsPsScript('C:\\Users\\<username>');
...
test('windowsSpawnCmd: guarded ssh argv carrying only an EncodedCommand (no quotes to mangle)', () => {
  ...
  assert.deepEqual(sshArgs.slice(0, 5), ['-o', 'BatchMode=yes', '-o', 'ConnectTimeout=5', 'windows-node']);
  ...
  assert.match(script, /-WorkingDirectory 'C:\\Users\\<username>'/);
});
...
test('spawnWindows: dry-run returns contract shape and spawns nothing', async () => {
  ...
  for (const s of r.spawned) {
    ...
    assert.match(s.cmd, /ssh windows-node powershell -NoProfile -NonInteractive -EncodedCommand <base64>/);
