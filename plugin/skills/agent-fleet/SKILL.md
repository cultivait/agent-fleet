...
> - `Operator`, `operator`, and `referee` are **reserved callsigns** — the hub rejects `/register` for these names (403). No agent can impersonate the operator by claiming one.
...
> - Check the fleet at **task boundaries only** (not in a continuous loop) — call `fleet_standby` once, handle any messages, then return to your task.
> - **Never enter the standby loop while doing project work.** The loop in Step 2 is for dedicated interactive chat sessions only.
> - **Don't narrate progress to your terminal.** The board auto-publishes your activity and mission to all agents — post a terse fleet line or update `fleet_mission` instead.
> - **On claiming or being assigned a task, create a `TodoWrite` list and keep it updated as you work.** The Cockpit Live view renders your todos as step-by-step progress — an instance with no todo list shows as active-but-blank (`todos: null`), so the operator can't see how far along you are. Same rule as "don't narrate": put progress in the task list (auto-published), not in terminal prose.
